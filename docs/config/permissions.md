...
+ 在`simpleui`中没有提供自定义权限的支持。
+ 在原生`django admin`中没有提供自定义菜单权限的支持，自定义按钮权限支持。SimplePro进行了扩展

# 权限基础知识

`simplepro`的权限 也是在django admin的权限中进行扩展的。但是未更改django 权限相关的任何字段。

::: tip

核心的原理也只不过是往以下三个表中添加数据，然后通过`request.user.get_all_permissions()`方法获取权限，进行权限判断。

当然你也可以重写`request.user.get_all_permissions()`方法，自定义权限判断逻辑。
:::

重写User教程->：[User.get_all_permissions()方法](/config/rewrite_user)
